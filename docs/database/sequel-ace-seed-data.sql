-- Sequel Ace 직접 실행용 GoingGoing MVP 데이터 입력 쿼리.
-- 실행 위치: Sequel Ace > goinggoing 연결 > Query 탭
-- 실행 방법: 이 파일 전체 복사 > Query 탭에 붙여넣기 > Run All
-- 선행 조건: docs/database/goinggoing-mariadb.sql로 테이블 생성 완료
-- 포함 데이터: 지역, 테마, 하위 카테고리, 태그, 컨텐츠, 카드뉴스, 태그 연결, 추천 루트
